...
import { fetchSchools, fetchSchool, toggleVisited } from "../api";
import {
  FiArrowLeft,
  FiPhone,
  FiSearch,
...
export default function Schools() {
  ...
  const handleViewDetails = async (udise) => {
    if (!udise || udise === "all" || udise.length < 5) {
  ...
  const handleToggleVisited = async (udise, visited) => {
    try {
      await toggleVisited(udise, visited);
      toast.success(visited ? "Marked as visited" : "Removed from visited");

      if (visited) {
        setSchools((prev) => prev.filter((s) => s.udise_code !== udise));
        setFilteredSchools((prev) => prev.filter((s) => s.udise_code !== udise));
      } else {
        loadSchools();
      }
      ...
      if (selectedSchool) {
        setSelectedSchool({
          ...selectedSchool,
          school: { ...selectedSchool.school, visited },
        });
      }
    } catch {
    ...
    const all = [
      {
        ...
        distance: calculateDistance(
          userLoc.lat,
          userLoc.lng,
          selectedSchool.school.latitude,
          selectedSchool.school.longitude
        ),
      },
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto">
        ...
        >
          ...
          <span className="font-semibold">
            Back to {decodeURIComponent(name)} Schools
          </span>
        </button>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <span className="bg-white/20 px-3 py-1 rounded-full text-sm">
                  <FiHome className="inline mr-1" /> {school.district}
                </span>
                <span className="bg-white/20 px-3 py-1 rounded-full text-sm">
            ...
            <div className="flex flex-col gap-2 mt-4 md:mt-0 w-full md:w-auto">
              ...
              >
                {findingLocation ? (
                  <>
                    <FiRefreshCw className="mr-2 animate-spin" /> Finding...
                  </>
                ) : userLocation ? (
                  <>
                    <FiTarget className="mr-2" /> Location Found
                  </>
                ) : (
                  <>
        ...
        {school.visited ? (
          <div className="bg-yellow-50 border-2 border-yellow-300 rounded-xl p-6 mb-8">
            <div className="flex items-start">
              <FiAlertCircle className="text-yellow-600 mt-1 mr-3 flex-shrink-0" size={24} />
              <div className="flex-1">
                <h3 className="text-lg font-bold text-yellow-800 mb-2">
                  This school is marked as visited
                </h3>
                <p className="text-yellow-700 mb-4">
                  It is hidden from district listings and nearby searches.
                </p>
                <button
                  onClick={() => handleToggleVisited(school.udise_code, false)}
          ...
          <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-6 mb-8">
            <div className="flex items-start">
              <FiCheck className="text-blue-600 mt-1 mr-3" size={24} />
              <div className="flex-1">
                <h3 className="text-lg font-bold text-blue-800 mb-2">
                  Not yet visited
                </h3>
                <p className="text-blue-700 mb-4">
                  Mark this school as visited to hide it from public listings.
                </p>
                <button
                  onClick={() => handleToggleVisited(school.udise_code, true)}
        ...
        {locationError && !userLocation && (
          <div className="bg-red-50 border-2 border-red-200 rounded-xl p-6 mb-8">
            <p className="text-red-800 font-medium">
              ⚠️ Location access issue. Please enable location and try again.
            </p>
          </div>
        )}
                  ...
                  {nearbyFromUser.map((s, i) => (
                    <div key={s.udise_code || i} className="bg-white p-4 rounded-lg border">
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-bold ${getDistanceColor(
                            s.distance
                          )}`}
                        >
                      ...
                      <div className="mt-2 flex gap-2">
                        {s.udise_code !== school.udise_code && (
                          <button
                            onClick={() => handleViewDetails(s.udise_code)}
                            className="text-xs bg-blue-50 px-3 py-1 rounded"
                          >
                            View
                          </button>
                        )}
                        <button
                          onClick={(e) =>
                            openDirections(s.latitude, s.longitude, s.school_name, e)
                          }
                          className="text-xs bg-green-50 px-3 py-1 rounded"
                        >
                          Directions
                        </button>
                      </div>
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className="mt-1 px-3 py-1 bg-green-100 text-green-700 text-sm rounded"
                        >
                          Call
                        </button>
                      </div>
                  ...
                  {nearby.map((ns) => (
                    <div key={ns.udise_code} className="border-2 rounded-lg p-4 hover:shadow-md">
                      ...
                      <div className="mt-3 flex gap-2">
                        <button
                          onClick={() => handleViewDetails(ns.udise_code)}
                          className="px-3 py-1 bg-blue-50 text-blue-700 text-xs rounded"
                        >
                          View
                        </button>
                        <button
                          onClick={(e) => openMap(ns.location_url, ns.school_name, e)}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                <button
                  onClick={closeDetails}
                  className="w-full flex items-center justify-center px-4 py-3 border-2 border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50"
                >
                  <FiArrowLeft className="mr-2" /> Back to List
                </button>
                <button
                  onClick={getUserLocationForDetail}
                  className="w-full flex items-center justify-center px-4 py-3 bg-orange-500 text-white font-semibold rounded-lg hover:bg-orange-600"
                >
                  <FiCompass className="mr-2" /> Get My Location
                </button>
                <button
                  onClick={(e) => callSchool(school.mobile, school.school_name, e)}
                  className="w-full flex items-center justify-center px-4 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block w-full flex items-center justify-center px-4 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700"
                >
                ...
                {userLocation && (
                  <button
                    ...
                    className="w-full flex items-center justify-center px-4 py-3 bg-green-500 text-white font-semibold rounded-lg hover:bg-green-600"
                  >
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-6">
              ...
              <p className="text-sm text-blue-800 mb-2">
                <strong>Name:</strong> {school.school_name}
              </p>
              <p className="text-sm text-blue-800 mb-2">
                <strong>PIN:</strong> {school.pincode}
              </p>
      ...
      {filteredSchools.length ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredSchools.map((school) => {
            if (!school.udise_code || school.udise_code === "all" || school.udise_code.length < 5) {
              return null;
            }
            return (
              <div
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  {userLocation && school.distance !== null && (
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-bold whitespace-nowrap ${getDistanceColor(
                        school.distance
                      )}`}
                    >
                ...
                {userLocation && school.latitude && school.longitude && (
                  <div className="flex flex-wrap gap-2 mt-2 mb-3">
                    <button
                      onClick={(e) =>
                        openDirections(school.latitude, school.longitude, school.school_name, e)
                      }
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <div className="flex flex-wrap gap-4 text-xs text-gray-600">
            <div className="flex items-center">
              <div className="w-3 h-3 bg-green-500 rounded-full mr-2"></div> &lt;1 km
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-yellow-500 rounded-full mr-2"></div> 1–3 km
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-orange-500 rounded-full mr-2"></div> 3–5 km
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-red-500 rounded-full mr-2"></div> &gt;5 km
            </div>
          </div>
        </div>
      )}
